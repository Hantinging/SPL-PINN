...
from datasetheat import get_dataloader
from datasetheat import get_testloader
...
def draw_loss(Loss_list,epoch):
    ...
    plt.grid()
    plt.savefig("heatwithspl_evalloss.png")
    plt.show()
...
def train():
    ...
    u_real = np.sin(np.pi * x) * np.exp(-np.pi * np.pi * t)
    mse = mean_absolute_error(u, u_real)
    ti = np.zeros(25600)
    ...
    mse_b = mean_absolute_error(ub0, u_real_b) + mean_absolute_error(ub1, u_real_b)
